(function (root, factory) {
  ...
  var SEMANTIC = Object.freeze({
    ...
    reviewTiming: ['shopping_now', 'renewal_60', 'later', 'not_sure']
  });
  var BRANCHES = Object.freeze({
    owner_occupied: Object.freeze({ key: 'owner_occupied', destinationType: 'coveragefit', destination: '/assessment/', propertyRequired: true, label: 'the home you own and live in' }),
    landlord: Object.freeze({ key: 'landlord', destinationType: 'coveragefit', destination: '/assessment/', propertyRequired: true, label: 'your rental property' }),
    buyer: Object.freeze({ key: 'buyer', destinationType: 'coveragefit', destination: '/assessment/', propertyRequired: true, label: 'the home you are buying' }),
    renter: Object.freeze({ key: 'renter', destinationType: 'renters', destination: '/contact/?intent=renters', propertyRequired: false, label: 'your renters review' })
  });
  ...
  function renderRecovery() {
    ...
    if (copy) copy.textContent = isHandoff
      ? (state.destinationType === 'renters' ? 'Your renters path is saved in this tab. Continue to Dylan’s renters contact options without submitting the form again.' : 'Your contact permission and review context are already connected in this tab. Continue to CoverageFit without submitting the form again.')
      : 'Your three quick-question answers are saved in this tab. No contact or property details were stored in this checkpoint.';
    if (continueButton) continueButton.textContent = isHandoff ? (state.destinationType === 'renters' ? 'Continue to Renters Options' : 'Continue to CoverageFit') : 'Continue My Review';
