/* 408-LIFE-1.7 — engagement + application-initialization UI. Network transport lives only in life-secure-submit.js. */
(function (window, document) {
  ...
  function initIntake() {
    ...
    function isValid(step) {
      ...
      if (number === 4) return validRequiredText(step) && validGender(step) && validDateOfBirth(step);
      if (number === 5) return validRequiredText(step) && validZip(step) && validEmail(step) && validPhone(step);
      if (number === 6) return validLast4(step) && validAcknowledgement(step);
      return false;
    }
    ...
    function syncControls(step) {
      ...
      step.querySelectorAll('[data-life-next],[data-life-secure-submit]').forEach(function (button) {
    ...
    function finishSecureSubmission() {
      ...
      announce('Application-start details were securely received.');
    ...
    document.addEventListener('life:secure-submission-complete', function () { finishSecureSubmission(); });
