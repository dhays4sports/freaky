/* 408-LIFE-1.7 — protected producer queue UI. Memory-only; no browser persistence. */
(function (window, document) {
  ...
  function renderDetail(item) {
    ...
    detail('legal_name', legal); detail('gender', label(a.gender)); detail('date_of_birth', formatDob(a.date_of_birth)); detail('ssn_last4', a.ssn_last4 ? '***-**-' + a.ssn_last4 : '—');
  ...
  async function setItemStatus(status) {
    ...
    try {
      ...
      if (state.openItem) { state.openItem.status = data.status; renderDetail(state.openItem); }
